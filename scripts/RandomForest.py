...
class RandomForest:
    ...
    def train(self, folds=1):
        ...
        acc_arr = []
        self.logger.info(
            f"Model RandomForestClassifier training started with k-flod: {folds}")
    ...
    def __printAccuracy(self, acc, step=1, label=""):
        self.logger.info(f"Model RandomForestClassifier accuracy: {acc}")
        print(
            f"step {step}: {label} Accuracy of RandomForestClassifier is: {acc:.3f}")

    def __printLoss(self, loss, step=1, label=""):
        self.logger.info(f"Model RandomForestClassifier accuracy: {loss}")
        print(
            f"step {step}: {label} Loss of RandomForestClassifier is: {loss:.3f}")
